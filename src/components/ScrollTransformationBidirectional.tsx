...
// BIDIRECTIONAL VERSION: Mathematically correct for both directions
export const ScrollTransformationBidirectional: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // MATHEMATICAL APPROACH: Use absolute scroll positions
      const currentScrollY = window.scrollY;
      const sectionTop = currentScrollY + rect.top; // Absolute position of section top
      const sectionBottom = sectionTop + sectionHeight;
      
      // Define sticky range based on absolute positions
      const stickyStartY = sectionTop - headerHeight; // When section top hits header
      const stickyEndY = sectionBottom - windowHeight; // When section bottom hits bottom of viewport
      
      // Sticky logic: same calculation regardless of scroll direction
      const shouldBeSticky = currentScrollY >= stickyStartY && currentScrollY <= stickyEndY;
      setIsSticky(shouldBeSticky);
      
      // Progress logic: based on position within sticky range
      let progress = 0;
      if (currentScrollY >= stickyStartY && currentScrollY <= stickyEndY) {
        const scrolledIntoStickyRange = currentScrollY - stickyStartY;
        const totalStickyRange = stickyEndY - stickyStartY;
        progress = totalStickyRange > 0 ? scrolledIntoStickyRange / totalStickyRange : 0;
      ...
      // Debug information
      setDebugInfo(`ScrollY: ${Math.round(currentScrollY)} | StickyStart: ${Math.round(stickyStartY)} | StickyEnd: ${Math.round(stickyEndY)} | Range: ${Math.round(stickyEndY - stickyStartY)}`);
    };
  ...
  return (
    <section 
      ...
      style={{ height: '300vh' }}
    >
      <div className="fixed top-4 left-4 z-50 bg-cyan-600 text-white px-3 py-1 rounded text-sm">
        BIDIRECTIONAL: Math-Based
      </div>
              ...
              <Card className="p-6 bg-gradient-to-r from-emerald-50 to-blue-50 border-2 border-emerald-200 shadow-lg">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-gray-600 max-w-md mx-auto">
                    BIDIRECTIONAL: Mathematisch korrekte Berechnungen für beide Scroll-Richtungen
                  </p>
