...
export const ComplianceTransformation: React.FC = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowSolutions(true), 2000);
    ...
    <section className="py-20 bg-gradient-to-b from-white via-gray-50/50 to-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16 space-y-6">
          <Badge className="bg-red-500/10 text-red-600 border-red-500/20 text-sm">
        ...
        <div className="relative">
          ...
          <div className="flex justify-center mb-8">
            ...
            >
              {showSolutions ? "Probleme anzeigen" : "Lösungen anzeigen"}
            </Button>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
            {transformationCards.map((card, index) => (
              <div key={card.id} className="relative min-h-[280px]">
                {/* Problem Card */}
                <div
                  className={cn(
                    "absolute inset-0 transition-all duration-500",
                    showSolutions ? "opacity-0 scale-95 pointer-events-none" : "opacity-100 scale-100"
                  )}
                >
                  <Card className={cn(
                    "h-full p-4 border-2",
                    `bg-gradient-to-br ${card.problem.bgGradient}`,
                    "border-red-200 hover:border-red-300 shadow-md hover:shadow-lg"
                  )}>
                    <div className="flex flex-col h-full space-y-3">
                      <div className="flex items-center gap-2">
                        <div className="p-2 rounded-lg bg-red-500/20">
                          <card.problem.icon className={cn("h-5 w-5", card.problem.color)} />
                        </div>
                ...
                <div
                  className={cn(
                    "absolute inset-0 transition-all duration-500",
                    showSolutions ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
                  )}
                >
                  <Card className={cn(
                    "h-full p-4 border-2",
                    `bg-gradient-to-br ${card.solution.bgGradient}`,
                    "border-emerald-200 hover:border-emerald-300 shadow-md hover:shadow-lg"
                  )}>
                    <div className="flex flex-col h-full space-y-3">
                      <div className="flex items-center gap-2">
                        <div className="p-2 rounded-lg bg-emerald-500/20">
                          <card.solution.icon className={cn("h-5 w-5", card.solution.color)} />
                        </div>
