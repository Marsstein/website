...
// LÖSUNG 2: Scroll-Richtungs-Detection
export const ScrollTransformationTest2: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Verschiedene Logik je nach Scroll-Richtung
      if (direction === 'down') {
        // Beim Runterscrollen: früher sticky werden
        const shouldBeSticky = rect.top <= headerHeight && rect.bottom > windowHeight;
        setIsSticky(shouldBeSticky);
        
        // Progress basiert auf Sektion die in View kommt
        if (rect.top <= headerHeight && rect.bottom > headerHeight) {
          const scrolledPastHeader = Math.max(0, headerHeight - rect.top);
          const maxScrollDistance = sectionHeight - windowHeight;
          const progress = Math.min(scrolledPastHeader / maxScrollDistance, 1);
          setScrollProgress(progress);
        } else if (rect.top > headerHeight) {
          setScrollProgress(0);
        }
      } else {
        // Beim Hochscrollen: später sticky werden
        const shouldBeSticky = rect.top <= headerHeight * 0.5 && rect.bottom > windowHeight * 1.2;
        setIsSticky(shouldBeSticky);
        
        // Progress umgekehrt berechnen
        if (rect.bottom >= windowHeight && rect.top <= headerHeight) {
          const scrolledFromBottom = Math.max(0, windowHeight - rect.bottom);
          const maxScrollDistance = sectionHeight - windowHeight;
          const progress = Math.max(0, 1 - (scrolledFromBottom / maxScrollDistance));
          setScrollProgress(progress);
        }
      }
    };
  ...
  return (
    <section 
      ...
      style={{ height: '300vh' }}
    >
      <div className="fixed top-4 left-4 z-50 bg-orange-500 text-white px-3 py-1 rounded text-sm">
          ...
          <div className="container mx-auto max-w-6xl px-4 py-8">
            ...
            <div className="text-center mb-6 space-y-3">
              ...
              <div className="text-sm text-gray-500">
                Progress: {Math.round(scrollProgress * 100)}% | Sticky: {isSticky ? 'Yes' : 'No'} | Direction: {scrollDirection}
              </div>
            </div>
              ...
              <Card className="p-6 bg-gradient-to-r from-emerald-50 to-blue-50 border-2 border-emerald-200 shadow-lg">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-gray-600 max-w-md mx-auto">
                    Test 2: Scroll-Richtungs-Detection mit verschiedener Logik für up/down
                  </p>
