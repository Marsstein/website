...
// APPROACH 3: State Machine + predictive scroll direction
export const ScrollTransformationApproach3: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (!sectionRef.current || boundaries.start === 0) return;
      ...
      // STATE MACHINE: Eindeutige Zustände
      let newState: ScrollState = 'before';
      let progress = 0;

      if (currentScrollY < boundaries.start) {
        newState = 'before';
        progress = 0;
      } else if (currentScrollY > boundaries.end) {
        newState = 'after';
        progress = 1;
      } else {
        newState = 'sticky';
        // Linear progress through sticky range
        const scrolledIntoRange = currentScrollY - boundaries.start;
        const totalRange = boundaries.end - boundaries.start;
        progress = totalRange > 0 ? scrolledIntoRange / totalRange : 0;
      }

      // PREDICTIVE: Adjust for scroll direction momentum
      if (direction === 'down' && newState === 'sticky') {
        // Slightly accelerate progress when scrolling down
        progress = Math.min(progress * 1.1, 1);
      } else if (direction === 'up' && newState === 'sticky') {
        // Slightly decelerate progress when scrolling up
        progress = Math.max(progress * 0.95, 0);
      }
      ...
      // Debug info
      setDebugInfo(`Approach3 | ScrollY: ${Math.round(currentScrollY)} | State: ${newState} | Direction: ${direction} | Range: ${Math.round(boundaries.start)}-${Math.round(boundaries.end)}`);
    };
  ...
  return (
    <section 
      ...
      style={{ height: '300vh' }}
    >
      <div className="fixed top-4 left-4 z-50 bg-purple-600 text-white px-3 py-1 rounded text-sm">
        APPROACH 3: State Machine
      </div>
      
      <div className={cn(
        "w-full transition-all duration-100", // Very fast transition
          ...
          <div className="container mx-auto max-w-6xl px-4 py-8">
            ...
            <div className="text-center mb-6 space-y-3">
              ...
              <div className="text-xs text-gray-500 max-w-3xl mx-auto">
                Progress: {Math.round(scrollProgress * 100)}% | State: {scrollState}<br/>
                {debugInfo}
              </div>
            </div>
            ...
            <div className="w-full h-2 bg-gray-200 rounded-full mb-6 max-w-md mx-auto">
              <div 
                className="h-full bg-gradient-to-r from-red-500 via-amber-500 to-emerald-500 rounded-full transition-all duration-100"
              ...
              <Card className="p-6 bg-gradient-to-r from-emerald-50 to-blue-50 border-2 border-emerald-200 shadow-lg">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-gray-600 max-w-md mx-auto">
                    APPROACH 3: State Machine mit Scroll-Direction Prediction
                  </p>
