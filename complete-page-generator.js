...
/**
 * Complete Page Generator - Generiert vollständige HTML-Seiten mit Header und Footer
 */
...
// Tools Page Complete Content
const TOOLS_COMPLETE_CONTENT = `
  ${HEADER_TEMPLATE}
...
/**
 * Generiert vollständige Seite mit Header und Footer
 */
async function generateCompletePage(route, title, description, mainContent) {
  // Bereinige den Main Content
  const cleanedContent = mainContent
    .replace(/<main[^>]*>/g, '') // Entferne main opening tag
    .replace(/<\/main>/g, '')    // Entferne main closing tag
    .trim();
  
  // Erstelle vollständige Seite
  const completePage = `
    ${HEADER_TEMPLATE}
...
/**
 * Aktualisiert die Tools-Seite mit vollständigem Content
 */
async function updateToolsPage() {
  console.log('🔧 Aktualisiere Tools-Seite mit vollständigem Content...');
  
  const toolsModulePath = path.join(__dirname, 'scripts/content/tools/overview.mjs');
  
  // Schreibe vollständiges Tools-Modul
  const toolsModule = `export default {
  route: '/tools',
  ...
  content: \`${TOOLS_COMPLETE_CONTENT.replace(/`/g, '\\`')}\`
};`;
  
  fs.writeFileSync(toolsModulePath, toolsModule);
  console.log('   ✅ Tools-Seite aktualisiert');
  
  // Aktualisiere Index
  const indexPath = path.join(__dirname, 'scripts/content/tools/index.mjs');
  const indexContent = `import overview from './overview.mjs';
import compliance_ai_assistant from './compliance_ai_assistant.mjs';
import cookie_management from './cookie_management.mjs';
import dsgvo_compliance_scorecard from './dsgvo_compliance_scorecard.mjs';
import dsgvo_email_template_generator from './dsgvo_email_template_generator.mjs';
...
export default {
  ...
  console.log('   ✅ Tools Index aktualisiert');
}

// Hauptfunktion
async function main() {
  console.log('🚀 Starte Complete Page Generator...\n');
  
  await updateToolsPage();
  
  console.log('\n✨ Fertig! Führe Build aus...');
