    ...
    try {
      await page.waitForFunction(() => {
        ...
        if (window.React && window.ReactDOM) {
          // Prüfe ob wichtige Elemente da sind
          const hasHeader = document.querySelector('header') !== null;
          const hasMain = document.querySelector('main') !== null;
          const hasContent = root.textContent.length > 100;
          
          return hasHeader || hasMain || hasContent;
        }
        
        return false;
      }, { timeout: 5000 });
    } catch (e) {
      console.warn(`  ⚠️  Timeout waiting for React on ${route}, continuing anyway`);
    }
...
function validateHtml(html, route) {
  ...
  // Prüfe ob React Root existiert und nicht leer ist
  const rootMatch = html.match(/<div id="root"[^>]*>(.*?)<\/div>/s);
  if (!rootMatch || rootMatch[1].trim().length < 10) {
    console.warn(`  ⚠️  Empty or missing root element on ${route}`);
    return false;
  }
...
async function startPreviewServer() {
  ...
  try {
    ...
    try {
      ...
      // Server läuft nicht, starte einen neuen
    }
    
    // Starte Server im Hintergrund
    execSync(`npx vite preview --port ${CONFIG.PORT} > /dev/null 2>&1 &`, {
      cwd: paths.context === 'netlify' ? rootDir : process.cwd(),
      stdio: 'ignore'
    });
